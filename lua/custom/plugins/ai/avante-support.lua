...
--    - Storage: ~/.config/nvim/lua/custom/plugins/ai/system-prompts.json
--
...
function M.provider_select()
  ...
  local providers = { 'copilot', 'claude', 'gemini', 'openai' }
    ...
    vim.ui.select(models, {
      ...
      vim.ui.select({ 'Yes', 'No' }, {
        ...
        if make_default == 'Yes' then
          ...
          vim.notify('Switched to ' .. selected_provider .. '/' .. selected_model, vim.log.levels.INFO)
        end
      end)
    end)
  end)
...
function M.setup_commands()
  ...
  vim.api.nvim_create_user_command('AvantePrompt', function()
    local ok, system_prompts = pcall(require, 'custom.plugins.ai.system-prompts')
    if ok then
